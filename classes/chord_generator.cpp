...
Chord* ChordGenerator::getMajorChord(string root_name) {
	...
	if (chord_index == -1) {
		...
		Chord* output = new Chord(chord_name, &(note_array->at(root_index)), &(note_array->at(third_index)), &(note_array->at(fifth_index)));
...
int ChordGenerator::searchNote(string name) {
	for (int i = 0; i < 12; i++) {
		if (note_array->at(i).note_name.compare(name) == 0) {
			return i;
		}
	}
	return -1;
